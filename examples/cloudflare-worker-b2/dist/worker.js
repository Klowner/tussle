...
class B2Auth {
    constructor(options) {
        ...
        operators.switchMap(() => b2AuthorizeAccount.b2AuthorizeAccountRequest({
            ...
            applicationKey,
            applicationKeyId,
        })), operators.tap((auth) => console.log(auth)));
        this.state$ = this.response$.pipe(operators.flatMap((response) => _esm5.from(response.getData())), operators.shareReplay({ refCount: true, bufferSize: 1 }));
    }
...
function createUploadURLPool(b2, options) {
    const create = () => b2
        ...
        .pipe(operators.mergeMap((res) => _esm5.from(res.getData())))
        .toPromise();
...
const _hasatob$1 = typeof atob === 'function';
const _hasbtoa$1 = typeof btoa === 'function';
const _hasBuffer$1 = typeof Buffer === 'function';
const b64ch$1 = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=';
const b64re$1 = /^(?:[A-Za-z\d+\/]{4})*?(?:[A-Za-z\d+\/]{2}(?:==)?|[A-Za-z\d+\/]{3}=?)?$/;
const _fromCC$1 = String.fromCharCode.bind(String);
const _U8Afrom$1 = typeof Uint8Array.from === 'function'
    ? Uint8Array.from.bind(Uint8Array)
    : (it, fn = (x) => x) => new Uint8Array(Array.prototype.slice.call(it, 0).map(fn));
const _mkUriSafe$1 = (src) => src
    .replace(/[+\/]/g, (m0) => m0 == '+' ? '-' : '_')
    .replace(/=+$/m, '');
const _tidyB64$1 = (s) => s.replace(/[^A-Za-z0-9\+\/]/g, '');
/**
 * does what `window.btoa` of web browsers do.
 * @param {String} bin binary string
 * @returns {string} Base64-encoded string
 */
const _btoa$1 = _hasbtoa$1 ? (bin) => btoa(bin)
    : _hasBuffer$1
        ...
        : (bin) => {
            ...
            for (let i = 0; i < bin.length;) {
                if ((c0 = bin.charCodeAt(i++)) > 255 ||
                    ...
                    throw new TypeError('invalid character found');
                u32 = (c0 << 16) | (c1 << 8) | c2;
                asc += b64ch$1.charAt(u32 >> 18 & 63)
                    + b64ch$1.charAt(u32 >> 12 & 63)
                    + b64ch$1.charAt(u32 >> 6 & 63)
                    + b64ch$1.charAt(u32 & 63);
            }
            return pad ? asc.slice(0, pad - 3) + "===".substring(pad) : asc;
        };
const _fromUint8Array$1 = _hasBuffer$1
    ? (u8a) => Buffer.from(u8a).toString('base64')
    : (u8a) => {
        // cf. https://stackoverflow.com/questions/12710001/how-to-convert-uint8-array-to-base64-encoded-string/12713326#12713326
        const maxargs = 0x1000;
        let strs = [];
        for (let i = 0, l = u8a.length; i < l; i += maxargs) {
            strs.push(_fromCC$1.apply(null, u8a.subarray(i, i + maxargs)));
        }
 ...
 * @returns {string} Base64 string
 */
const fromUint8Array$1 = (u8a, rfc4648 = false) => rfc4648 ? _mkUriSafe$1(_fromUint8Array$1(u8a)) : _fromUint8Array$1(u8a);
/**
 * @deprecated should have been internal use only.
 * @param {string} src UTF-8 string
 * @returns {string} UTF-16 string
 */
const utob$1 = (src) => unescape(encodeURIComponent(src));
/**
 *
 */
const _encode$1 = _hasBuffer$1
    ? (s) => Buffer.from(s, 'utf8').toString('base64')
    : (s) => _btoa$1(utob$1(s));
/**
 * converts a UTF-8-encoded string to a Base64 string.
 ...
 * @returns {string} Base64 string
 */
const encode$1 = (src, rfc4648 = false) => rfc4648
    ? _mkUriSafe$1(_encode$1(src))
    : _encode$1(src);
/**
 ...
 * @returns {string} UTF-8 string
 */
const btou$1 = (src) => decodeURIComponent(escape(src));
/**
 * does what `window.atob` of web browsers do.
 * @param {String} asc Base64-encoded string
 * @returns {string} binary string
 */
const _atob$1 = _hasatob$1 ? (asc) => atob(_tidyB64$1(asc)) :
    _hasBuffer$1 ? (asc) => Buffer.from(asc, 'base64').toString('binary') :
        (asc) => {
            asc = asc.replace(/\s+/g, '');
            if (!b64re$1.test(asc))
                throw new TypeError('malformed base64.');
            ...
            let u24, bin = '', r1, r2;
            for (let i = 0; i < asc.length;) {
                u24 = b64ch$1.indexOf(asc.charAt(i++)) << 18
                    | b64ch$1.indexOf(asc.charAt(i++)) << 12
                    | (r1 = b64ch$1.indexOf(asc.charAt(i++))) << 6
                    | (r2 = b64ch$1.indexOf(asc.charAt(i++)));
                bin += r1 === 64 ? _fromCC$1(u24 >> 16 & 255)
                    : r2 === 64 ? _fromCC$1(u24 >> 16 & 255, u24 >> 8 & 255)
                        : _fromCC$1(u24 >> 16 & 255, u24 >> 8 & 255, u24 & 255);
            }
            return bin;
        };
const _decode$1 = _hasBuffer$1
    ? (a) => Buffer.from(a, 'base64').toString('utf8')
    : (a) => btou$1(_atob$1(a));
const _unURI$1 = (a) => _tidyB64$1(a.replace(/[-_]/g, (m0) => m0 == '-' ? '+' : '/'));
/**
 * converts a Base64 string to a UTF-8 string.
 * @param {String} src Base64 string.  Both normal and URL-safe are supported
 * @returns {string} UTF-8 string
 */
const decode$1 = (src) => _decode$1(_unURI$1(src));
/**
 * converts a Base64 string to a Uint8Array.
 */
const toUint8Array$1 = _hasBuffer$1
    ? (a) => _U8Afrom$1(Buffer.from(_unURI$1(a), 'base64'))
    : (a) => _U8Afrom$1(_atob$1(_unURI$1(a)), c => c.charCodeAt(0));
const _noEnum$1 = (v) => {
    return {
        value: v, enumerable: false, writable: true, configurable: true
    };
 ...
 * extend String.prototype with relevant methods
 */
const extendString$1 = function () {
    const _add = (name, body) => Object.defineProperty(String.prototype, name, _noEnum$1(body));
    _add('fromBase64', function () { return decode$1(this); });
    _add('toBase64', function (rfc4648) { return encode$1(this, rfc4648); });
    _add('toBase64URI', function () { return encode$1(this, true); });
    _add('toBase64URL', function () { return encode$1(this, true); });
    _add('toUint8Array', function () { return toUint8Array$1(this); });
};
/**
 * extend Uint8Array.prototype with relevant methods
 */
const extendUint8Array$1 = function () {
    const _add = (name, body) => Object.defineProperty(Uint8Array.prototype, name, _noEnum$1(body));
    _add('toBase64', function (rfc4648) { return fromUint8Array$1(this, rfc4648); });
    _add('toBase64URI', function () { return fromUint8Array$1(this, true); });
    _add('toBase64URL', function () { return fromUint8Array$1(this, true); });
};
/**
 * extend Builtin prototypes with relevant methods
 */
const extendBuiltins$1 = () => {
    extendString$1();
    extendUint8Array$1();
};
const gBase64$1 = {
    ...
    extendString: extendString$1,
    extendUint8Array: extendUint8Array$1,
    extendBuiltins: extendBuiltins$1
};
...
var js_base64_1$1 = getCjsExportFromNamespace(base64$1);
...
class TussleOutgoingCloudflareFetchResponse {
    constructor(request, originalResponse) {
        ...
        this.getData = () => __awaiter(this, void 0, void 0, function* () {
            const response = yield originalResponse.json();
            return response;
        });
    }
}
    ...
    makeRequest(request) {
        ...
        if (proxyRequest) {
            ...
            if (request.auth) {
                ...
                newRequestInit.headers['Authorization'] = 'Basic ' + js_base64_1$1.Base64.encode([
